...
namespace ClassifiedAds.Infrastructure.Identity
{
                             ...
                             IUserTwoFactorRecoveryCodeStore<User>
    {
        ...
        {
            return Task.FromResult(_userRepository.Get(new QueryOptions { IncludeTokens = true }).FirstOrDefault(x => x.NormalizedEmail == normalizedEmail));
        }

        public Task<User> FindByIdAsync(string userId, CancellationToken cancellationToken)
        {
            return Task.FromResult(_userRepository.Get(new QueryOptions { IncludeTokens = true }).FirstOrDefault(x => x.Id == Guid.Parse(userId)));
        }

        public Task<User> FindByNameAsync(string normalizedUserName, CancellationToken cancellationToken)
        {
            return Task.FromResult(_userRepository.Get(new QueryOptions { IncludeTokens = true }).FirstOrDefault(x => x.NormalizedUserName == normalizedUserName));
        }
